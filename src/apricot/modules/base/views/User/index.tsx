 ...
 * @LastEditTime: 2021-10-27 02:34:22
...
import { Menu, Avatar, Tag } from 'antd';
...
try {
  // 获取用户数据
  // baseServices.system.user
  //   .page({ page: 1, size: 20, sort: 'desc', order: 'createTime' })
  //   .then((res) => {
  //     const { list } = res as unknown as ListRes;
  //     console.log('🚀 ~ file: index.tsx ~ line 30 ~ baseServices.system.user.page ~ res', res);
  //     tableListDataSource = list.map((r: TableListItem) => {
  //       return { ...r, key: r.id };
  //     });
  //   });
  // 获取部门数据
  baseServices.system.dept.list({}).then((result: any) => {
    result?.data?.map((r: any) => {
      deptList[r.id] = r.name;
    });
    // deptListDataSource = toTreeData(result.data);
    deptListDataSource = toTreeData(result.data, {
      id: 'id',
      parentId: 'parentId',
    }) as any;
      ...
      <ProCard>
        <ProTable<TableListItem>
          ...
          request={async (
            ...
            console.log('🚀 ~ file: index.tsx ~ line 213 ~ User ~ filter', filter);
            console.log('🚀 ~ file: index.tsx ~ line 213 ~ User ~ sort', sort);
            console.log('🚀 ~ file: index.tsx ~ line 213 ~ User ~ params', params);
            await waitTime(200);
